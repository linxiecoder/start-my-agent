...
from startmyagent.chapters.chapter6.AgentScopeDemo.game_roles import GameRoles
from startmyagent.chapters.chapter6.AgentScopeDemo.prompt_cn import ChinesePrompt
from startmyagent.chapters.chapter6.AgentScopeDemo.utils_cn import MAX_DISCUSSION_ROUND, majority_vote_cn, \
    GameModerator, format_player_list, check_winning_cn, MAX_GAME_ROUND, get_chinese_name
from structured_output_cn import (
    DiscussionModelCN, WerewolfKillModelCN, get_seer_model_cn, get_vote_model_cn, get_hunter_model_cn,
    WitchActionModelCN
)
    ...
    def update_alive_players(self, dead_players: List[str]):
        ...
        for dead_name in dead_players:
            if dead_name:
                ...
                self.villages = [p for p in self.villages if p.name != dead_name]
