# 游戏常量
import random
from typing import List, Dict, Counter, Optional, Any
...
def get_chinese_name(character: str = None):
    """获取角色中文名"""
...
def check_winning_cn(alive_players: List[AgentBase], roles: Dict[str, str]) -> Optional[str]:
    """检查中文本游戏胜利条件"""
    alive_roles = [roles.get(player.name, "村民") for player in alive_players]
...
class GameModerator(AgentBase):
    ...
    async def announce(self, content: str) -> str:
        """发布游戏公告"""
        msg = Msg(
            name=self.name,
            content=f"📢 {content}",
